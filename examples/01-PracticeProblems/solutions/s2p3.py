...
[x, y, oh], [u, v, pp] = p.add_neural_network(
    name="NN", inputs=["x", "y", "oh"], outputs=["u", "v", "p"]
)
...
subr = p.Rectangle("subr1", (w / 2 - 0.1, 0), (w / 2 + 0.1, oh), params=params)
...
inlet_outlet = p.add_boundary_subdomain(
    "inlet_outlet", geom=r1, criteria=Eq(x, 0) | Eq(x, w), params=params
)

int_constrict = p.add_boundary_subdomain(
    "int_constrict", geom=int_rect, criteria=Eq(x, 0) | Eq(x, w / 2), params=params
)
...
p.add_constraint(
    "mass_balance_middle",
    enforce(equation=Eq(Integral(nx * u + ny * v, x, y), 0), on_domain=int_constrict),
)
