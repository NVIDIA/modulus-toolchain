...
class FNOProblem:
    ...
    def set_grid_domain(self, N: int, extent):
        ...
        self._grid["vars"] = {
            sp.Symbol(vn): {
                ...
                "delta": (float(ve[1]) - float(ve[0])) / float(N),
            }
            for vn, ve in extent.items()
        }
    ...
    def add_interior_constraint(self, name, equation, over=None, criteria=None):
        assert (
            name not in self._constraints
        ), f"Constraint named [{name}] already exists, choose a different name"
        assert (
